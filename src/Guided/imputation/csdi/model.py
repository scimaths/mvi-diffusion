...
class CSDI(BaseNNImputer):
    ...
    def __init__(
        ...
        is_cls_free_guided : bool = False,
        cond_drop_prob_total: float = 0.1,
        cond_drop_prob_individual: float = 0.2,
        ...
        self.drop_prob_tot = cond_drop_prob_total
        self.drop_prob_iid = cond_drop_prob_individual
        self.cls_free = is_cls_free_guided
        ...
        self.model = _CSDI(
            ...
            is_cls_free_guided,
            cond_drop_prob_total,
            cond_drop_prob_individual,
            guidance_strength,
    ...
    def _assemble_input_for_training(self, data: list) -> dict:
        ...
        if self.cls_free:
            cond_mask = cond_mask * torch.bernoulli(torch.zeros_like(cond_mask) + (1-self.drop_prob_iid)).to(cond_mask.device)
            cond_mask = cond_mask * torch.bernoulli(torch.tensor(1-self.drop_prob_tot)).to(cond_mask.device)
    ...
    def _train_model(
        ...
        #print("sampling times ", n_sampling_times)
    ...
    def fit(
        ...
        self._train_model(training_loader, val_loader, n_sampling_times = n_sampling_times)
